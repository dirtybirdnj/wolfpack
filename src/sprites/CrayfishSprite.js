...
export class CrayfishSprite extends OrganismSprite {
    ...
    findNearbyZooplankton() {
        if (!this.scene.zooplankton)
            return [];
        return this.scene.zooplankton.filter((zp) => {
            if (!zp.visible || zp.consumed)
                return false;
            const distance = Phaser.Math.Distance.Between(this.worldX, this.y, zp.worldX, zp.y);
    ...
    handleHuntingBehavior(nearbyZooplankton) {
        ...
        if (this.currentTarget && this.currentTarget.visible && !this.currentTarget.consumed) {
            this.targetLockTime++;
            const currentDist = Phaser.Math.Distance.Between(this.worldX, this.y, this.currentTarget.worldX, this.currentTarget.y);
            ...
            if (currentDist < 150) {
                ...
                if (this.targetLockTime > this.minLockDuration) {
                    ...
                    nearbyZooplankton.forEach(zp => {
                        if (!zp.visible || zp.consumed || zp === this.currentTarget)
                            return;
                        const distance = Phaser.Math.Distance.Between(this.worldX, this.y, zp.worldX, zp.y);
        ...
        if (this.currentTarget) {
            ...
            const distance = Phaser.Math.Distance.Between(this.worldX, this.y, this.currentTarget.worldX, this.currentTarget.y);
...
//# sourceMappingURL=CrayfishSprite.js.map