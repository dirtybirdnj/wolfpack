...
export class NotificationSystem {
    ...
    showCatchNotification(fish) {
        ...
        const text = this.scene.add.text(GameConfig.CANVAS_WIDTH / 2, 240, `FISH ON!\n${info.weight}\n+${fish.points} points`, {
            fontSize: '19px',
            fontFamily: 'Courier New',
            color: '#ffff00',
            align: 'center',
    ...
    showFishHookedNotification() {
        const text = this.scene.add.text(GameConfig.CANVAS_WIDTH / 2, 160, 'FISH ON!\nTAP SPACEBAR OR R2 TO REEL!', {
            fontSize: '22px',
            fontFamily: 'Courier New',
            color: '#ffff00',
            align: 'center',
    ...
    showGamepadDisconnected() {
        ...
        title.setDepth(3001);
        // Warning message
        const message = this.scene.add.text(centerX, 210, 'Your controller has been disconnected.\n\n' +
        ...
        // Continue hint
        const hint = this.scene.add.text(centerX, 330, 'Press SPACEBAR or ESC to dismiss', {
            fontSize: '14px',
            fontFamily: 'Courier New',
            color: '#88ff88',
            stroke: '#000000',
    ...
    showMessage(title, description, y = 100) {
        const text = this.scene.add.text(GameConfig.CANVAS_WIDTH / 2, y, `${title}\n${description}`, {
            fontSize: '14px',
            fontFamily: 'Courier New',
            color: '#ffff00',
            align: 'center',
    ...
    createPauseOverlay() {
        ...
        const resumeButton = this.createPauseMenuButton(centerX, centerY, 'RESUME', () => this.togglePause());
        this.buttons.push(resumeButton);
        const tackleBoxButton = this.createPauseMenuButton(centerX, centerY + buttonSpacing, 'TACKLE BOX', () => this.openTackleBox());
        this.buttons.push(tackleBoxButton);
        const controlsButton = this.createPauseMenuButton(centerX, centerY + buttonSpacing * 2, 'CONTROLS', () => this.showControlsDialog());
        this.buttons.push(controlsButton);
        const mainMenuButton = this.createPauseMenuButton(centerX, centerY + buttonSpacing * 3, 'MAIN MENU', () => this.goToMainMenu());
        this.buttons.push(mainMenuButton);
        // Controls hint - use dynamic scale for positioning
        const hintText = this.scene.add.text(centerX, this.scene.scale.height - 60, 'D-Pad/Arrows: Navigate | X: Select | SELECT: Tackle Box | CIRCLE: Resume', {
            fontSize: '11px',
            fontFamily: 'Courier New',
            color: '#88ff88',
            stroke: '#000000',
    ...
    showControlsDialog() {
        ...
        const closeHint = this.scene.add.text(centerX, this.scene.scale.height - 30, 'Press X, ENTER, or CIRCLE to close', {
            fontSize: '12px',
            fontFamily: 'Courier New',
            color: '#88ff88',
            stroke: '#000000',
