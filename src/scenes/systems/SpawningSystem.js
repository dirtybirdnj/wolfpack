...
export class SpawningSystem {
    ...
    spawnEmergencyFish() {
        ...
        let y = Utils.randomBetween(GameConfig.DEPTH_ZONES.MID_COLUMN.min * depthScale, GameConfig.DEPTH_ZONES.MID_COLUMN.max * depthScale);
        // Validate Y is within canvas bounds (safety check)
        const canvasHeight = this.scene.game.canvas.height;
        const waterFloorY = GameConfig.getWaterFloorY(canvasHeight);
        if (y > waterFloorY) {
        ...
        const text = this.scene.add.text(canvasWidth / 2, 200, 'HUNGRY FISH APPEARED!', {
            fontSize: '20px',
    ...
    triggerEmergencyFrenzy(emergencyFish) {
        ...
        const canvasWidth = this.scene.scale.width;
        const text = this.scene.add.text(canvasWidth / 2, 180, 'FEEDING FRENZY!', {
            fontSize: '24px',
            fontFamily: 'Courier New',
            color: '#ff0000',
            align: 'center',
