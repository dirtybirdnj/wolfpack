...
export class InputSystem {
    ...
    private mouseReeling: boolean;
    private mouseReelIntensity: number;
    private gamepadState: GamepadButtonState | null;
    ...
    constructor(scene: Phaser.Scene) {
        ...
        this.mouseReelIntensity = 1.0;

        // Initialize keys (will be properly set in setupInput)
        this.cursors = {} as Phaser.Types.Input.Keyboard.CursorKeys;
        this.spaceKey = {} as Phaser.Input.Keyboard.Key;
        this.rKey = {} as Phaser.Input.Keyboard.Key;
        this.escKey = {} as Phaser.Input.Keyboard.Key;
        this.pKey = {} as Phaser.Input.Keyboard.Key;
        this.qKey = {} as Phaser.Input.Keyboard.Key;
        this.eKey = {} as Phaser.Input.Keyboard.Key;
        this.vKey = {} as Phaser.Input.Keyboard.Key;
    ...
    setupGamepad(): void {
        ...
        if (!(window as any).gamepadManager) {
        ...
        (window as any).gamepadManager.on('connected', (gamepad: any) => {
    ...
    handleKeyboardInput(): void {
        ...
        if (this.cursors.up!.isDown) {
            (this.scene as any).lure.retrieve();
        } else if (this.mouseReeling) {
            // Use variable speed retrieve with mouse wheel intensity
            (this.scene as any).lure.retrieveWithTrigger(this.mouseReelIntensity);
        } else {
            (this.scene as any).lure.stopRetrieve();
        }

        // Adjust retrieve speed with left/right arrows
        if (Phaser.Input.Keyboard.JustDown(this.cursors.left!)) {
            (this.scene as any).lure.adjustSpeed(-1);
        ...
        if (Phaser.Input.Keyboard.JustDown(this.rKey)) {
            (this.scene as any).lure.reset();
        }

        // Drag adjustment with Q/E keys
        if ((this.scene as any).reelModel) {
            // Q key - decrease drag
            if (Phaser.Input.Keyboard.JustDown(this.qKey)) {
                (this.scene as any).reelModel.adjustDrag(-10);
                console.log(`Drag decreased to ${(this.scene as any).reelModel.dragSetting}% (${(this.scene as any).reelModel.getCurrentDragForce().toFixed(1)} lbs)`);
            }

            // E key - increase drag
            if (Phaser.Input.Keyboard.JustDown(this.eKey)) {
                (this.scene as any).reelModel.adjustDrag(+10);
                console.log(`Drag increased to ${(this.scene as any).reelModel.dragSetting}% (${(this.scene as any).reelModel.getCurrentDragForce().toFixed(1)} lbs)`);
            }
        }

        // V key - toggle vision range visualization for predators
        if (Phaser.Input.Keyboard.JustDown(this.vKey)) {
            (this.scene as any).visionRangeDebug = !(this.scene as any).visionRangeDebug;
            const status = (this.scene as any).visionRangeDebug ? 'ON' : 'OFF';
    ...
    handleGamepadInput(): void {
        ...
        const dpadUpBtn = (window as any).gamepadManager.getButton('DpadUp');
        const dpadDownBtn = (window as any).gamepadManager.getButton('DpadDown');
        const dpadLeftBtn = (window as any).gamepadManager.getButton('DpadLeft');
        const dpadRightBtn = (window as any).gamepadManager.getButton('DpadRight');
        const l1Btn = (window as any).gamepadManager.getButton('L1');
        const r1Btn = (window as any).gamepadManager.getButton('R1');
        const aBtn = (window as any).gamepadManager.getButton('X'); // X button on PS4 = A on Xbox
        const bBtn = (window as any).gamepadManager.getButton('Circle'); // Circle on PS4 = B on Xbox
        const xBtn = (window as any).gamepadManager.getButton('Square'); // Square on PS4 = X on Xbox
        const yBtn = (window as any).gamepadManager.getButton('Triangle'); // Triangle on PS4 = Y on Xbox

        // Get analog stick axes
        const leftStickX = (window as any).gamepadManager.getAxis(0); // Left stick X
        const leftStickY = (window as any).gamepadManager.getAxis(1); // Left stick Y
        ...
        const r2Trigger = (window as any).gamepadManager.getButton('R2');
        ...
        if (r2Trigger.value > triggerThreshold) {
            ...
            (this.scene as any).lure.retrieveWithTrigger(r2Trigger.value);
        } else {
        ...
        if (xBtn.pressed && !this.gamepadState!.lastX) {
            (this.scene as any).debugMode = !(this.scene as any).debugMode;
        }
        ...
        const rightStickY = (window as any).gamepadManager.getAxis(3); // Right stick Y axis
        (this.scene as any).lure.applyJig(rightStickY, DEAD_ZONE);
    ...
    handleFishFightInput(): number {
        ...
        if ((window as any).gamepadManager && (window as any).gamepadManager.isConnected()) {
            const r2Button = (window as any).gamepadManager.getButton('R2');
            const l1Button = (window as any).gamepadManager.getButton('L1');
            const r1Button = (window as any).gamepadManager.getButton('R1');
    ...
    checkPauseInput(): boolean {
        ...
        // Check gamepad
        if ((window as any).gamepadManager && (window as any).gamepadManager.isConnected()) {
            const startBtn = (window as any).gamepadManager.getButton('Start');
    ...
    updateSpeedDisplay(): void {
        const lureInfo = (this.scene as any).lure.getInfo();
