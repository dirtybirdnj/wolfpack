...
interface WaterColumnInitData {
    depthConverter: DepthConverter;
    maxDepth?: number;
}

interface BottomProfilePoint {
    x: number;
    y: number;
    type: 'normal' | 'structure';
}

interface Thermocline {
    depth: number;
    strength: number;
}
...
export default class WaterColumn extends Phaser.Scene {
    ...
    private maxDepth: number = GameConfig.MAX_DEPTH;
    private canvasWidth: number = 0;
    private canvasHeight: number = 0;
    private backgroundRT!: Phaser.GameObjects.RenderTexture;
    private graphics!: Phaser.GameObjects.Graphics;
    private bottomProfile: BottomProfilePoint[] = [];
    private thermoclines: Thermocline[] = [];
    private depthTexts: Phaser.GameObjects.Text[] = [];

    constructor() {
        super({ key: 'WaterColumn' });
    }

    init(data: WaterColumnInitData): void {
     ...
     */
    private generateBottomProfile(): BottomProfilePoint[] {
        const profile: BottomProfilePoint[] = [];
     ...
     */
    private drawBackgroundGradient(graphics: Phaser.GameObjects.Graphics): void {
     ...
     */
    private drawBottomProfile(graphics: Phaser.GameObjects.Graphics): void {
     ...
     * Create depth marker texts
     */
    private createDepthMarkers(): void {
