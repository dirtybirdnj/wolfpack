...
export class SchoolManager {
    ...
    updateSchools(allFish, fishType) {
        ...
        schoolMap.forEach(school => {
            // Get active members
            const activeMembers = Array.from(school.members).filter(fish => fish.active && fish.visible && !fish.consumed);
    ...
    getSchoolCount(fishType) {
        if (fishType === 'baitfish')
            return this.baitfishSchools.size;
        if (fishType === 'predator')
            return this.predatorSchools.size;
    ...
    removeFishFromSchool(fish) {
        if (!fish.schooling || !fish.schooling.schoolId)
            return;
